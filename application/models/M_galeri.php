...
class M_galeri extends CI_Model{
	...
	function simpan_galeri1($album_id,$user_id,$user_nama,$gambar){
		 $this->db->trans_start();
            $this->db->query("INSERT INTO tbl_galeri(galeri_album_id,galeri_pengguna_id,galeri_author,galeri_gambar) VALUES ('$album_id','$user_id','$user_nama','$gambar')");
            $this->db->query("UPDATE tbl_album SET album_count=album_count+1 WHERE album_id='$album_id'");
        $this->db->trans_complete();
        if($this->db->trans_status()==true)
        return true;
        else
	...
	function get_galeri_by_album_id_withlimit($idalbum,$limit){
